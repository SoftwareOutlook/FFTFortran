...
contains
  ...
  subroutine fft_bench(comm,n1,n2,n3,C,fftlib,check,flag,tm_fft_init,tm_fft,&
    ...
    complex(kind=wp), allocatable :: Dk(:,:), work(:,:), local(:), workmkl(:)
    ...
    case (3) ! MKL
       ...
       do k=1,n3     
          ...
          if (check) then
             ...
             if (my_id .eq. 0) then
                do i=1,n1
                   do j=1,n2
                      ...
                      Dk(i,j) = X(i,j)/real(n1*n2,kind=wp)
                   end do
                end do
             ...
             if (k.eq.n3 .and. my_id.eq. 0) then
...
20  select case (flag)
  ...
  subroutine fft_bench_fftw_mpi(n1,n2,n3,C,check,flag,tm_fft_init,tm_fft,&
       tm_ifft_init,tm_ifft,local_n2,local_j_offset)
    integer, intent(in) :: n1,n2,n3 ! Array dimensions
    complex (kind=wp), intent(in) :: C(n1,n2,n3) ! Input array 
    logical, intent(in) :: check ! Additionally, perform inverse, element-wise
    ...
    complex(C_DOUBLE_complex), dimension(n1,local_n2) :: in, iout
 ...
 !   tm_ifft = 0.0_wp
    ...
    plan = fftw_mpi_plan_dft_2d(n2_4,n1_4, in,out,comm,FFTW_FORWARD,flags)
    call mpi_barrier(comm,ierr)
    !$          tm2 = omp_get_wtime()
    tm_fft_init = tm_fft_init + tm2 - tm1
    if (check) then
