PROGRAM commandline
  ...
  integer :: comm, ierr, my_id, no_proc ! mpi variable
...
contains
  ...
  subroutine fft_bench(comm,n1,n2,n3,C,fftlib,check,flag,tm_fft_init,tm_fft,&
    ...
    case (1)
       ...
       !        if (k.eq.1) then
       !$          tm1 = omp_get_wtime()
       call ZFFT3D(Dk1,n1,n2,n3,0)
       !$          tm2 = omp_get_wtime()
       tm_fft_init = tm_fft_init + tm2 - tm1
       !        end if 
       !$      tm1 = omp_get_wtime()
       call ZFFT3D(Dk1,n1,n2,n3,-1)
       !$      tm2 = omp_get_wtime()
    ...
    case (3) ! MKL
       ...
       !$      tm1 = omp_get_wtime()
          ...
          rootrank=0
          status = mkl_cdft_scatterdata_d(comm,rootrank,elementsize,3,      &
               [n1,n2,n3],x,nx,start_x,local)
          if (status .ne. 0) then
             if (.not. dftierrorclass(status,dfti_no_error)) then
                write(*,*) 'error: ', dftierrormessage(status)
             endif
...
!                write(*,*) X(1:2,1:2,1:2)
       ...
       if (check) then
          ...
          do i=1,n1
             do j=1,n2
                do k=1,n3
                   Dk1(i,j,k) = X(i,j,k)/real(n1*n2*n3,kind=wp)
                end do
             end do
          end do
         if (my_id .eq.0) then
...
20  select case (flag)
  ...
  subroutine fft_bench_fftw_mpi(comm,n1,n2,n3,C,check,flag,tm_fft_init,tm_fft,&
    ...
    complex(C_DOUBLE), dimension(n1,n2,local_n3) :: in, iout
    complex(C_DOUBLE), dimension(n1,n2,local_n3) :: out, iin
    ...
    plan = fftw_mpi_plan_dft_3d(n3_4,n2_4,n1_4, in,out,comm,FFTW_FORWARD,flags)
    call mpi_barrier(comm,ierr)
    !$          tm2 = omp_get_wtime()
    tm_fft_init = tm_fft_init + tm2 - tm1
    if (check) then
       ...
       !$   tm1 = omp_get_wtime()
       iplan = fftw_mpi_plan_dft_3d(n3_4,n2_4,n1_4, iin,iout,comm,FFTW_BACKWARD,flags)
       call mpi_barrier(comm,ierr)
