...
!!$ tm2=omp_get_wtime()
  ...
  tm2 = real(nq,kind=wp)
  if (my_id .eq. 0) then
     write(*,'(a8,6i8,8e10.3e2)') "average",no_proc,fftlib,i,n1,n2,nq,&
          tm_fft_init_tot/tm2,&
...
contains
  ...
  subroutine fft_bench(comm,n1,n2,c,fftlib,check,flag,tm_fft_init,tm_fft,&
    ...
    complex(kind=wp), allocatable :: dk(:,:), work(:,:), work1(:,:)
    ...
    case (1)
       ...
       allocate(dk(1,n1_local),stat=stat)
       if (stat .ne. 0) then
          flag = -2
          goto 20
       end if
       allocate(work(1,n1_local),stat=stat)
       if (stat .ne. 0) then
          flag = -2
          goto 20
       end if
       ...
       do k=1,n2
          ...
          do i=1,n1_local
             ...
             dk(1,i) = c(i+my_id*nn,k)
                  write(*,*) my_id,i,dk(i,1)!, c(i+my_id*nn,k)
          ...
          if (k.eq.1) then
             !$          tm1 = omp_get_wtime()
             call pzfft2d(dk,work,1,n1,comm,npu,0)
             !$          tm2 = omp_get_wtime()
             tm_fft_init = tm_fft_init + tm2 - tm1
             
          end if
          !$   tm1 = omp_get_wtime()
          call pzfft2d(dk,work,1,n1,comm,npu,-1)
          !$   tm2 = omp_get_wtime()
          ...
          if (check) then
             ...
             !$        tm1 = omp_get_wtime()
             call pzfft2d(work,dk,1,n1,comm,npu,1)
             !$        tm2 = omp_get_wtime()
          ...
          !      do j=1,n2                                                                                                                                                           
                 ...
                 if (npu .eq. 1) then
                  ...
                  write(*,*) 'iout',my_id,i,dk(1,i), work1(i,1), c(i+my_id*nn,k)
