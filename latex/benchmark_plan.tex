...
For input data that is purely real, the discrete Fourier transform satisfies the ``Hermitian'' redundancy: in 1D, if $x$ is a real array, then $z$ computed via (\ref{Eqn:fft}) will be a complex array satisfying
...
\end{eqnarray*} 
This more efficient storage is known as half-complex and can be extended to more dimensions. Note that if the input vector $x$ is half-complex format, then $z$ will be a real vector.
...
The datatypes listed are real (R), complex (C) and half-complex (H). 
...
FFTE & R $\rightarrow$ H & 2,3   & $2^a\times 3^b \times 5^c$ & OpenMP, MPI, CUDA & Open source & \cite{FFTE} \\
     & C $\rightarrow$ C & 1,2,3 & & & & \\
     & H $\rightarrow$ R & 2,3   & & & & \\ \hline
FFTW & R $\rightarrow$ H & Any   & Any but optimised for  & Multithreading, MPI & GPL v3 & \cite{FFTW} \\
     & C $\rightarrow$ C & Any   & $2^a\times 3^b \times 5^c \times 7^d \times 11^e \times 13^f$ & & & \\
     & H $\rightarrow$ R & Any   & with $e+f = 0$ or $1$ & & & \\ \hline

\end{tabular}
\caption{Libraries being benchmarked, }\label{Tbl:libs}
\end{small}
\end{center}
\end{table}
